    ...
    const handleQuantityChange = (itemId: string, delta: number) => {
        if (id === 'cart-shipment') {
            ...
            setDemoQuantities(prev => ({
                ...prev,
                [itemId]: Math.max(0, (prev[itemId] ?? (shipment.items.find(i => i.id === itemId)?.pcs ?? 1)) + delta)
            }));
        }
    };
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-start justify-between">
                                            <div className="min-w-0 flex-1">
                                                <h4 className="text-[16px] font-bold text-[#181725] leading-tight">
                                                    {item.name}
                                                </h4>
                                                <p className="text-[12px] text-[#7C7C7C] font-medium mt-1">
                                                    {item.size} * {item.pcs} pc
                                                </p>
                                                <p className="text-[11px] text-[#53B175] font-bold mt-1">
                                        ...
                                        <div className="flex items-center justify-end mt-2">
                                            <div className="flex items-center border border-[#E2E2E2] rounded-full px-2 py-0.5 bg-white scale-90 origin-right">
                                                <button
                                                    onClick={() => {
                                                        if (!isDemo && item.pcs <= 1) removeFromCart(item.id);
