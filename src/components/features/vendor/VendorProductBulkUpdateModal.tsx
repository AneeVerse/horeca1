...
/**
 * VendorProductBulkUpdateModal — Vendor Bulk Product Update Modal
 * ---------------------------------------------------------------
 * Premium dialog to bulk adjust pricing, status, categories, and brands
 * directly from the Vendor products list page. Scoped strictly to the session vendor.
 */
...
export default function VendorProductBulkUpdateModal({ open, onClose, onComplete }: Props) {
  const confirm = useConfirm();

  const [filter, setFilter] = useState<FilterState>({ categoryId: '', brand: '', isActive: '' });
  ...
  useEffect(() => {
    if (open) {
      setFilter({ categoryId: '', brand: '', isActive: '' });
  ...
  useEffect(() => {
    ...
    fetch('/api/v1/brands')
      ...
      .then((j) => {
        ...
        setBrands(arr.map((b: { name?: string }) => ({ name: b.name ?? '' })).filter((b: BrandOption) => b.name));
      })
      .catch(() => {});
  }, [open]);
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[12px] text-[#7C7C7C] font-semibold mt-0.5">
                  Mass-edit prices, status, categories, and inventory metrics on items in your own catalog.
                </p>
              </div>
            </div>
            <button
