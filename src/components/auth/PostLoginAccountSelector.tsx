...
import { ShieldCheck, Store, Sparkles, User, MapPin, Loader2, X } from 'lucide-react';
...
export function PostLoginAccountSelector() {
  ...
  const { accounts, currentAccount, switchAccount, switching } = useBusinessAccountSwitcher();
  ...
  }, [status, accounts.length, session?.user?.id]);
